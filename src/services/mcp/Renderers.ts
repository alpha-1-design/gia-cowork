...
import { rendererRegistry, type MCPContentRenderer, type Renderer } from './RendererRegistry';
...
/**
 * Register the rich content renderers (images, video, audio, JSON, markdown,
 * code) into the shared renderer registry. Called once at app bootstrap.
 * More specific content types are registered first so the registry's
 * first-match-wins lookup prefers them over the generic defaults.
 */
export function registerRichRenderers(): void {
  // The registry expects function renderers `(item, container) => void`; the
  // rich renderers here expose an object API, so adapt them on registration.
  const adapt = (r: MCPContentRenderer): Renderer => (item, container) => {
    void r.render(item as MCPStructuredResult, container);
  };
  rendererRegistry.register('image/svg', adapt(SVGRenderer));
  rendererRegistry.register('image/gif', adapt(GIFRenderer));
  rendererRegistry.register('image/', adapt(ImageRenderer));
  rendererRegistry.register('video/', adapt(VideoRenderer));
  rendererRegistry.register('audio/', adapt(AudioRenderer));
  rendererRegistry.register('application/json', adapt(JSONRenderer));
  rendererRegistry.register('text/markdown', adapt(MarkdownRenderer));
  rendererRegistry.register('text/plain', adapt(TextRenderer));
  rendererRegistry.register('code/', adapt(CodeRenderer));
}
